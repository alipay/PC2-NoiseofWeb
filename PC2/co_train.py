...
def main(gpu, ngpus_per_node, opt):
    ...
    if opt.model_path:
        if os.path.isfile(opt.model_path):
            ...
            if opt.ctt_dir=="":
                opt.ctt_dir = checkpoint["opt"].output_dir
            if opt.resume:
                ...
                bank_name_A = "warmup_distri_bank_A.pkl"
                bank_name_B = "warmup_distri_bank_B.pkl"
            if os.path.exists(opt.ctt_dir) and os.path.isfile(os.path.join(opt.ctt_dir, bank_name_A)) and os.path.isfile(os.path.join(opt.ctt_dir, bank_name_B)):
                print("=> resume distribution bank from '{}'".format(opt.ctt_dir))
                with open(os.path.join(opt.ctt_dir, bank_name_A), 'rb') as f:
                    distri_bank_A = pickle.load(f)
                with open(os.path.join(opt.ctt_dir, bank_name_B), 'rb') as f:
