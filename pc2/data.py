    ...
    def __init__(
        ...
        if data_split == "train":
            ...
            if self.mode == "labeled":
                ...
                self.label = [pred[i].astype(int) for i in split_idx]
        ...
        if self.data_split == "train":
            if self.mode == "labeled":
                return (
                    ...
                    idx_ori,
                    torch.Tensor([1]), 
                    # torch.Tensor(np.array([self.label[index]])),
                    torch.Tensor(np.array([self.probability[index]])),
                    torch.Tensor(np.array([self.ctt_label[index]])),
                    torch.Tensor(np.array([self.ctt_probability[index]])),
                    self._labels[index],
                )
            elif self.mode == "unlabeled":
                return image, text, index, idx_ori, self._labels[index], 0
            else:
...
def collate_fn(data):
    ...
    labels = None
    if len(data[0]) == 9:
        images, captions, ids, ids_ori, labels, prob, ctt_labels, ctt_prob, _labels = zip(*data)
        # Merge
        labels = torch.stack(labels, 0).long()
        # Merge
        prob = torch.stack(prob, 0)
        # Merge
        ctt_labels = torch.stack(ctt_labels, 0).long()
        # Merge
    ...
    for i, cap in enumerate(captions):
        ...
        text[i, :end] = cap[:end]
    if len(data[0]) == 9:
        return images, text, lengths, ids, ids_ori, labels, prob, ctt_labels, ctt_prob, _labels
    # elif len(data[0]) == 6:
    #     return images, text, lengths, ids, labels, prob, _labels
    elif len(data[0]) == 6:
        return images, text, lengths, ids, ids_ori, _labels
    else:
...
def get_loader(
    ...
    if data_split == "warmup":
        ...
        labeled_dataset = PrecompDataset(
            ...
            probability=prob,
            ctt_pred=ctt_pred,
            ctt_probability=ctt_probability
        )
